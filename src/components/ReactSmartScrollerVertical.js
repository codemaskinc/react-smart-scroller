...
export class ReactSmartScrollerVertical extends React.Component {
    ...
    onScrollbarClick({ clientY }) {
        const { padding } = this.state;
        const thumbRef = this.thumbRef.current;
        const overflowRef = this.overflowContainerRef.current;
        const shouldReturn = C.all(thumbRef, overflowRef, clientY >= ((C.extractNumberFromStyle(thumbRef.style.top) || 0) + overflowRef.getBoundingClientRect().top + padding.top), clientY <= ((C.extractNumberFromStyle(thumbRef.style.top) || 0) + overflowRef.getBoundingClientRect().top + thumbRef.offsetHeight + padding.top));
    ...
    renderThumb() {
        ...
        return (React.createElement(RectangleThumb, { ref: this.thumbRef, onMouseDown: this.onMouseDown, style: { height } }));
    }
    renderScrollbar() {
    ...
    render() {
        ...
        return (React.createElement("div", { style: style },
            React.createElement(Content, { style: { cursor }, ref: this.overflowContainerRef, onScroll: this.onOverflowContentScroll, onMouseDown: draggable ? this.onOverflowContentMouseDown : C.noop }, this.renderChildren()),
